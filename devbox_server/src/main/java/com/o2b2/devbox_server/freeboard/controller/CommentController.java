...
public class CommentController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<CommentsDTO> editComment(@PathVariable Long id, @RequestBody Comment comment,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        Long userId = userDetails.getUserEntity().getId();
        Comment updatedComment = commentService.editComment(id, comment, userId);
        CommentsDTO commentDTO = commentService.convertToDTO(updatedComment);
        return ResponseEntity.ok(commentDTO);
    }
